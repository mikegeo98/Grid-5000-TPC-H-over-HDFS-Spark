...
spark = SparkSession.builder.appName("join4_DFAPI").config("spark.executor.instances","4").config("spark.executor.cores","4").master("yarn").getOrCreate()
...
tmp2 = lineitem.rdd.getNumPartitions()
tmp3 = supplier.rdd.getNumPartitions()

print("Partitions no lineitem supplier",tmp2,tmp3)
